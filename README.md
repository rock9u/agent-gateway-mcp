...
## Using with Claude Desktop Globally

To use this MCP server with Claude Desktop (or other MCP clients) while running it locally:
...
This will start the server on `http://localhost:3000/mcp`.
...
- **Tool**: `ping` - Simple health check that responds with "pong"
- **Tool**: `start-notification-stream` - Sends periodic notifications
- **Prompt**: `greeting-template` - Generates greeting messages
- **Resource**: `greeting-resource` - Static greeting content

You can verify the connection by asking Claude to use the `ping` tool. The server logs will show "MCP server hit: [timestamp]" when Claude calls it.

### Notes

- The local server must be running for Claude to access it
- Changes to the server code require restarting `npm start`
- Check the server console logs to see when Claude interacts with your MCP server
